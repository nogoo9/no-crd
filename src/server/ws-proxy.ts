 ...
 *
 * Intercepts HTTP Upgrade requests targeting workspace routes (/route/:workspaceId/*)
 * and proxies TCP socket streams directly to target pod IPs.
 *
 * NOTE ON RUNTIME COMPATIBILITY:
 * In Node.js (node), socket data piping and 101 Switching Protocols upgrade handshakes
 * function with 100% fidelity.
 * In Bun (bun), Bun's native C++ HTTP engine (uWebSockets) drops upgraded TCP sockets
 * when manual 101 HTTP response bytes are written to server sockets.
 * For production workloads requiring terminal or GUI WebSocket proxying, run the server
 * under Node.js (e.g. `moon run mcp:deploy` or `node dist/index.js`).
 * See docs/developer/cross-runtime-design.md and oven-sh/bun#28871 for details.
 */
export function registerUpgradeHandler(
	...
	function handleUpgradeRequest(req: any, socket: any, head: any) {
		if (socket.parser) {
			socket.parser.onIncoming = null;
			socket.parser = null;
		}
		if (req.socket?.parser) {
			req.socket.parser.onIncoming = null;
			req.socket.parser = null;
		}
		...
		if (head && head.length > 0) {
			clientBuffer.push(head);
		}
		const onClientData = (chunk: any) => {
			if (
				upstreamSocket &&
		...
		(async () => {
			...
			if (config.auth.enabled && !isNoAuth) {
				...
				try {
					...
					if (
						...
						logger.warn(
							"WebSocket upgrade failed: Missing role {requiredRole}",
							{
								requiredRole,
							},
						);
			...
			upstreamSocket = net.connect(Number(port), podIP, () => {
				if (upstreamSocket && typeof upstreamSocket.setNoDelay === "function") {
