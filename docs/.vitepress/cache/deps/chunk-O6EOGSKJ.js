...
var _isVoidTag = makeMap(VOID_TAGS);
...
var _isKnownMathMLAttr = makeMap(
...
function handleError(err, instance, type, throwInDev = true) {
	...
	const { errorHandler, throwUnhandledErrorInProduction } =
		instance?.appContext.config || EMPTY_OBJ;
	if (instance) {
...
function withCtx(fn, ctx = currentRenderingInstance, _isNonScopedSlot) {
	if (!ctx) return fn;
	if (fn._n) {
		return fn;
	}
...
Server rendered element contains more child nodes than client vdom.`,
  ...
  - expected on client: ${clientText}`,
			...
			let vnodeHooks;
			if ((vnodeHooks = props?.onVnodeBeforeMount)) {
			...
			if (
				(vnodeHooks = props?.onVnodeMounted) ||
				dirs ||
				needCallTransitionHooks
			) {
...
function isMismatchAllowed(el, allowedType) {
	...
	const allowedAttr = el?.getAttribute(allowMismatchAttr);
	if (allowedAttr == null) {
		return false;
	} else if (allowedAttr === "") {
		return true;
	...
	setup(props, { slots }) {
		...
		sharedContext.activate = (
			...
			queuePostRenderEffect(() => {
				...
				const vnodeHook = vnode.props?.onVnodeMounted;
				if (vnodeHook) {
		...
		sharedContext.deactivate = (vnode) => {
			...
			queuePostRenderEffect(() => {
				...
				const vnodeHook = vnode.props?.onVnodeUnmounted;
				if (vnodeHook) {
					invokeVNodeHook(vnodeHook, instance2.parent, vnode);
				}
	...
	if (sourceIsArray || isString(source)) {
		...
		if (!Number.isInteger(source) || source < 0) {
			...
			for (let i = 0; i < source; i++) {
				ret[i] = renderItem(i + 1, i, void 0, cached?.[i]);
			}
		}
	} else if (isObject(source)) {
		if (source[Symbol.iterator]) {
			ret = Array.from(source, (item, i) =>
				renderItem(item, i, void 0, cached?.[i]),
			);
		} else {
			...
			for (let i = 0, l = keys.length; i < l; i++) {
				const key = keys[i];
				ret[i] = renderItem(source[key], key, i, cached?.[i]);
			}
		}
	} else {
		ret = [];
...
function defineExpose(_exposed) {
	if (true) {
		warnRuntimeUsage(`defineExpose`);
	}
}
function defineOptions(_options) {
	if (true) {
...
function withDefaults(_props, _defaults) {
	if (true) {
...
function normalizeEmitsOptions(comp, appContext, asMixin = false) {
	const cache =
		__VUE_OPTIONS_API__ && asMixin ? mixinEmitsCache : appContext.emitsCache;
	...
	if (rawProps) {
		for (const key in rawProps) {
			...
			if (options && hasOwn(options, (camelKey = camelize(key)))) {
				if (!needCastKeys?.includes(camelKey)) {
...
function normalizePropsOptions(comp, appContext, asMixin = false) {
	const cache =
		__VUE_OPTIONS_API__ && asMixin ? mixinPropsCache : appContext.propsCache;
	const cached = cache.get(comp);
	if (cached) {
		return cached;
	}
...
var normalizeObjectSlots = (rawSlots, slots, _instance) => {
...
function baseCreateRenderer(options, createHydrationFns) {
	...
	const patch = (
		...
		if (ref2 != null && parentComponent) {
			setRef(ref2, n1?.ref, parentSuspense, n2 || n1, !n2);
	...
	const processElement = (
		...
		if (n1 == null) {
			...
			const customElement = n1.el?._isVueCE ? n1.el : null;
			try {
				if (customElement) {
	...
	const setupRenderEffect = (
		...
		const componentUpdateFn = () => {
			if (!instance.isMounted) {
				...
				if ((vnodeHook = next.props?.onVnodeBeforeUpdate)) {
				...
				if ((vnodeHook = next.props?.onVnodeUpdated)) {
					queuePostRenderEffect(
						() => invokeVNodeHook(vnodeHook, parent, next, vnode),
						parentSuspense,
					);
	...
	const getNextHostNode = (vnode) => {
		...
		const teleportEnd = el?.[TeleportEndKey];
...
function triggerEvent(vnode, name) {
	const eventListener = vnode.props?.[name];
...
function isVNodeSuspensible(vnode) {
	const suspensible = vnode.props?.suspensible;
...
function isSameVNodeType(n1, n2) {
	if (n2.shapeFlag & 6 && n1.component) {
		...
		if (dirtyInstances?.has(n1.component)) {
			n1.shapeFlag &= -257;
			n2.shapeFlag &= -513;
			return false;
		}
		...
		for (const key in toMerge) {
			if (key === "class") {
				...
				if (
					...
					incoming == null &&
					existing == null && // mergeProps({ 'onUpdate:modelValue': undefined }) should not retain
					// the model listener.
					!isModelListener(key)
				) {
					ret[key] = incoming;
...
function finishComponentSetup(instance, isSSR, _skipOptions) {
	const Component = instance.type;
	if (!instance.render) {
...
var computed2 = (getterOrOptions, debugOptions) => {
	...
	if (true) {
		const i = getCurrentInstance();
		if (i?.appContext.config.warnRecursiveComputed) {
...
function initCustomFormatter() {
	...
	function isKeyOfType(Comp, key, type) {
		...
		if (Comp.mixins?.some((m) => isKeyOfType(m, key, type))) {
			return true;
		}
	}
...
var templateContainer = doc?.createElement("template");
var nodeOps = {
...
function resolveTransitionProps(rawProps) {
	...
	const leaveDuration = durations?.[1];
	const {
		onBeforeEnter,
		onEnter,
		onEnterCancelled,
...
function patchDOMProp(el, key, value, _parentComponent, attrName) {
...
function patchEvent(el, rawName, _prevValue, nextValue, instance = null) {
...
var VueElement = class _VueElement extends BaseClass {
	...
	connectedCallback() {
		...
		while (
			(parent =
				parent && // #12479 should check assignedSlot first to get correct parent
	...
	_resolveDef() {
		...
		const resolve2 = (def2, _isAsync = false) => {
			this._resolved = true;
			this._pendingResolve = void 0;
	...
	_setAttr(key) {
		...
		const camelKey = camelize(key);
		if (has && this._numberProps?.[camelKey]) {
	...
	_setProp(key, val, shouldReflect = true, shouldUpdate = false) {
		if (val !== this._props[key]) {
			...
			if (shouldReflect) {
				...
				ob?.observe(this, { attributes: true });
			}
		}
	}
	_update() {
	...
	return (
		...
		(cache[cacheKey] = (event, ...args) => {
			for (let i = 0; i < modifiers.length; i++) {
				...
				if (guard?.(event, modifiers)) return;
			}
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-O6EOGSKJ.js.map
